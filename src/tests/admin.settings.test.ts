...
const mockedGetSystemRestrictions =
  adminService.getSystemRestrictionsService as jest.Mock;
const mockedUpdateSystemRestrictions =
  adminService.updateSystemRestrictionsService as jest.Mock;
const mockedGetNotificationTemplates =
  adminService.getNotificationTemplatesService as jest.Mock;
const mockedUpdateNotificationTemplate =
  adminService.updateNotificationTemplateService as jest.Mock;
const mockedGetAdminProfile = adminService.getAdminProfileService as jest.Mock;
const mockedResetPasswordAdmin =
  adminService.resetPasswordAdminService as jest.Mock;
const mockedUpdateAdminPassword =
  adminService.updateAdminPasswordServive as jest.Mock;
const mockedAddTemplate = adminService.addTemplateService as jest.Mock;
...
describe("Settings API (/api/admin/settings)", () => {
  ...
  describe("GET /settings/system-restrictions", () => {
    it("should return 200 and the system restrictions", async () => {
      ...
      const response = await request(app).get(
        "/api/admin/settings/system-restrictions"
      );
    ...
    it("should return 400 if no data is provided", async () => {
      (
        jest.requireMock("../validations/auth.validation")
          .SystemRestrictionsUpdateSchema.parse as jest.Mock
  ...
  describe("GET /settings/profile/:userId", () => {
    it("should return 200 and the admin profile", async () => {
      ...
      const response = await request(app).get(
        `/api/admin/settings/profile/${mockAdminUserId}`
      );
  ...
  describe("PUT /settings/profile/:userId", () => {
    it("should return 200 and update the admin's profile data (text only)", async () => {
      ...
      const userInDb = await User.findById(mockAdminUserId);
    ...
    it("should return 200 and update profile with a file upload", async () => {
      ...
      expect(response.body.user.profile).toBe(
        "http://cloudinary.com/profile.png"
      );
  ...
  describe("PUT /settings/profile/password-reset/:userId", () => {
    it("should return 200 and set passwordResetRequired to true", async () => {
      ...
      const response = await request(app).put(
        `/api/admin/settings/profile/password-reset/${mockAdminUserId}`
      );
  ...
  describe("PUT /settings/profile/password/:userId", () => {
    it("should return 200 and update the admin's password", async () => {
      mockedUpdateAdminPassword.mockImplementation(
      ...
      const userInDb = await User.findById(mockAdminUserId).select("+password");
