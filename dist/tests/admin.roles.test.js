...
describe("Roles & Permissions API (/api/admin/roles, /api/admin/permissions)", () => {
    let testRole;
    let viewUsersPermission;
    let manageUsersPermission;
    beforeEach(async () => {
        [viewUsersPermission, manageUsersPermission] = await permission_model_1.Permission.insertMany([
    ...
    afterEach(async () => {
        ...
        await permission_model_1.Permission.deleteMany({});
    });
    /* ========================= GET /permissions ========================= */
    describe("GET /permissions", () => {
        it("should return 200 and a list of all available permissions", async () => {
            const response = await (0, supertest_1.default)(index_1.app).get("/api/admin/permissions");
    ...
    describe("GET /roles", () => {
        it("should return 200 and a list of all roles with populated permissions", async () => {
            const response = await (0, supertest_1.default)(index_1.app).get("/api/admin/roles");
            expect(response.status).toBe(200);
            ...
            expect(response.body.roles[0].permissions[0].permissionKey).toBe("admin:viewAllUsers");
        });
    ...
    describe("POST /roles", () => {
        ...
        it("should return 200 and the newly created role", async () => {
            ...
            const roleInDb = await role_model_1.default.findById(response.body.role._id);
            expect(roleInDb?.permissions).toHaveLength(2);
        });
        it("should return 400 if roleName is missing", async () => {
            auth_validation_1.RoleSchema.parse.mockImplementationOnce(() => {
                throw {
                    name: "ZodError",
            ...
            const response = await (0, supertest_1.default)(index_1.app)
                .post("/api/admin/roles")
                .send(badData);
            const serviceLevelResponse = await (0, supertest_1.default)(index_1.app)
                .post("/api/admin/roles")
    ...
    describe("PUT /roles/:roleId", () => {
        it("should return 200 and the updated role when changing permissions", async () => {
            ...
            const receivedKeys = response.body.data.permissions.map((p) => p.permissionKey);
    ...
    describe("DELETE /roles/:roleId", () => {
        it("should return 200 and a success message", async () => {
            const response = await (0, supertest_1.default)(index_1.app).delete(`/api/admin/roles/${testRole._id}`);
            expect(response.status).toBe(200);
            expect(response.body.message).toBe("Role deleted successfully");
            const roleInDb = await role_model_1.default.findById(testRole._id);
            expect(roleInDb).toBeNull();
        });
        it("should return 400 if roleId is not a valid ObjectId", async () => {
            const response = await (0, supertest_1.default)(index_1.app).delete("/api/admin/roles/invalid-id");
            expect(response.status).toBe(400);
            expect(response.body.error).toBe("Invalid userId");
        });
        it("should return 404 if the role ID does not exist", async () => {
            const validObjectId = new mongoose_1.default.Types.ObjectId().toString();
            const response = await (0, supertest_1.default)(index_1.app).delete(`/api/admin/roles/${validObjectId}`);
            expect(response.status).toBe(404);
            expect(response.body.error).toBe("Role not found.");
        });
        it("should return 400 if the role is currently assigned to any users", async () => {
            ...
            const response = await (0, supertest_1.default)(index_1.app).delete(`/api/admin/roles/${testRole._id}`);
            expect(response.status).toBe(400);
            expect(response.body.error).toContain("Cannot delete this role. It is currently assigned to");
            const roleInDb = await role_model_1.default.findById(testRole._id);
